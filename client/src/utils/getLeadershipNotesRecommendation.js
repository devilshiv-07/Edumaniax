// utils/getLeadershipNotesRecommendation.js
import { GoogleGenerativeAI } from "@google/generative-ai";
import { notesLeadership6to8 } from "@/data/notesLeadership6to8";

const genAI = new GoogleGenerativeAI(import.meta.env.VITE_API_KEY);

export const getLeadershipNotesRecommendation = async (userMistakes) => {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

    const prompt = `
    You are an educational tutor. A student just played a leadership game and made mistakes.
    ...
    return notesLeadership6to8.filter((n) => ids.includes(n.topicId));
  } catch (err) {
    console.error("Gemini recommendation error:", err);
    return [notesLeadership6to8[0]]; // fallback: Leadership basics
  }
};
