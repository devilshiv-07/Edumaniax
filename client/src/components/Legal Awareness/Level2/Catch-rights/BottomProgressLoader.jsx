...
const BottomProgressLoader = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full z-50 backdrop-blur bg-black/40 text-white pt-4 pb-6 px-4 lilita-one-regular border-t border-white/10 shadow-inner">
      <div className="text-center text-lg sm:text-xl md:text-2xl font-extrabold drop-shadow-sm mb-1">TIP:</div>
      <div className="text-center text-sm sm:text-base md:text-lg text-gray-200 drop-shadow-sm mb-4">
        Drag and drop each statement into Rights, Duties, or Laws before it hits the ground!
      </div>
        ...
        >
          {progress > 0 && (
            <div
              ...
              style={{ width: `${progress}%`, clipPath: `polygon(8px 0, 100% 0, calc(100% - 8px) 100%, 0 100%)` }}
            />
          )}
          {progress < 100 && progress > 0 && (
            <div
              className="absolute top-0 h-full bg-white"
              style={{ left: `calc(${progress}% - 2.5%)`, width: "5%", clipPath: "polygon(20% 0, 100% 0, 80% 100%, 0% 100%)" }}
            />
          )}
        </div>
        <div className="ml-3 text-white text-sm sm:text-base font-extrabold drop-shadow-[1px_1px_0px_#000] whitespace-nowrap">{progress}%</div>
      </div>
    </div>
  );
};
