...
const SESSION_STORAGE_KEY = 'nitrogenReactorGameState'; // Unique key for this game
...
const gameData = {
    sequence: { id: 1, title: "🔁 Repair the Cycle", prompt: "Nitrogen-fixing bacteria  ⟶ ___________ ⟶  Plants absorb nitrates", options: ["Nitrifying bacteria convert ammonia to nitrates", "Ammonification by decomposers", "Denitrification returns nitrogen", "Nitrogen Fixation repeats"], correctAnswer: "Nitrifying bacteria convert ammonia to nitrates" },
    mcq: { id: 2, title: "🌱 Field Scenario", prompt: "Overuse of urea causes algae blooms & fish death. Which process is accelerated?", options: ["Ammonification", "Nitrogen Fixation", "Eutrophication", "Nitrification"], correctAnswer: "Eutrophication" },
    matching: { id: 3, title: "🔍 Process Match-Up", prompt: "Match each nitrogen cycle process with its correct function", categories: ["Nitrogen Fixation", "Nitrification", "Ammonification", "Denitrification"], options: ["Converts N₂ to ammonia", "Converts ammonia to nitrates", "Converts organic waste to ammonia", "Converts nitrates to atmospheric N₂"], correctAnswers: { "Nitrogen Fixation": "Converts N₂ to ammonia", "Nitrification": "Converts ammonia to nitrates", "Ammonification": "Converts organic waste to ammonia", "Denitrification": "Converts nitrates to atmospheric N₂" } },
};
        ...
        <div className="w-full max-w-[1111px] bg-gray-800/30 rounded-xl p-6 md:py-5 md:px-6 lg:py-7 lg:px-10 inter-font">
            <div className="flex flex-col justify-center items-start gap-6 md:gap-4">
                <div className="px-1 flex flex-col justify-center items-start ">
                    <h2 className="text-slate-100 text-lg lg:text-xl font-medium leading-9">{gameData.matching.title}</h2>
                    <p className="text-slate-100 text-xs lg:text-sm font-medium leading-relaxed">{gameData.matching.prompt}</p>
                </div>
                        ...
                        <div key={category} className="w-full">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 justify-start items-stretch  gap-2.5 md:gap-2 lg:gap-3 ">
                                {gameData.matching.options.map(option => {
                                    ...
                                    return (
                                        <div key={option} onClick={() => handleSelect(category, option)} className={`w-full md:w-auto px-6 py-3 md:px-4 md:py-2 lg:px-6 lg:py-4  rounded-lg shadow-[0px_2px_0px_0px_rgba(55,70,79,1.00)] border flex items-center justify-center cursor-pointer transition-all ${isSelected ? "bg-[#202f36] border-[#5f8428] shadow-[0_2px_0_0_#5f8428]" : "bg-gray-900 border-gray-700 hover:bg-gray-800"}`}>
                                            <span className={`text-center text-xs lg:text-[12px] font-medium leading-relaxed ${isSelected ? "text-[#79b933]" : "text-slate-100"}`}>{option}</span>
                                        </div>
                                    )
                                })}
                            </div>
...
// Main Game Component
// =============================================================================
export default function NitrogenReactor() {
    ...
    useEffect(() => {
        if (gameState === 'end' && finalResults && !insight) {
            const generateInsight = async () => {
                ...
                const prompt = `A student completed a 3-part quiz on the Nitrogen Cycle. Their accuracy was ${finalResults.accuracy}%. Their specific results are: ${JSON.stringify(finalResults.review)}. The available notes are: ${JSON.stringify(noteSectionsForModule)}. TASK: 1. DETECT: Find the most relevant note section. The game is about the Nitrogen Cycle, part of biogeochemical cycles. 2. GENERATE: Write a 25-35 word insight recommending that section by title. OUTPUT: JSON with "detectedTopicId" and "insight".`;
    ...
    const handleNavigateToSection = () => {
        if (recommendedSectionId) {
            const stateToSave = { gameState, introStep, gameStep, sequenceAnswer, mcqAnswer, matchingAnswers, finalResults, insight, recommendedSectionId, recommendedSectionTitle };
