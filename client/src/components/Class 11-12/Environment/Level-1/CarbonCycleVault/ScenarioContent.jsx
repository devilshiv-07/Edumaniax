...
const scenarioData = {
    id: 1,
    question: "🔍 Fix the sequence",
    scenario: "Rearrange the scrambled steps of the carbon cycle into the correct order.",
    options: [
        "Respiration", "Combustion", "Photosynthesis", "Decomposition", "Ocean uptake",
    ],
};
...
const ScenarioContent = () => {
    ...
    useEffect(() => {
        const animationInterval = setInterval(() => {
            ...
            setMovingItem("Respiration");

            // 2. After a short delay, perform the swap in the list state
            setTimeout(() => {
                setListItems(currentItems => {
                    const newItems = [...currentItems];
                    const indexRespiration = newItems.indexOf("Respiration");
                    const indexCombustion = newItems.indexOf("Combustion");
