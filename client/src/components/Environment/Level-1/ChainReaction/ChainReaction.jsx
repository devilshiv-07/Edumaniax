...
// FIX: CSS to hide the scrollbar on all browsers
...
// =============================================================================
// DnD Components (Responsive with Touch Support - No Changes)
// =============================================================================
const DraggableCard = React.memo(({ id, content }) => {
    ...
    return (
        // CHANGE: Smaller height on mobile, larger on medium screens and up.
...
// =============================================================================
// Reducer Logic (No Changes)
// =============================================================================
const initialState = {
    gameState: "intro", currentPuzzleIndex: 0, score: 0, answers: [], timeLeft: TIME_PER_PUZZLE,
};
...
const ChainReaction = () => {
    ...
    const handleDragEnd = (event) => {
        ...
        } else if (isDraggingFromSequence && isDroppingOnAvailable) {
            const draggedContent = newSequenceSlotsContent[activeSequenceSlotIndex].id;
            if (draggedContent) {
    ...
    return (
        // FIX: Changed h-screen to min-h-screen and removed overflow-hidden to allow scrolling on mobile
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
                <div className="flex flex-col items-center gap-8 w-full max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row w-full justify-center items-start gap-4 md:gap-8">
                        ...
                        <div className="flex w-full md:w-80 lg:w-96 flex-col gap-3 p-4 bg-[rgba(32,47,54,0.3)] rounded-xl border border-[#37464f]">
                            {sequenceSlotsContent.map((item) => (
                                <DroppableSequenceSlot key={item.slotId} id={item.slotId} content={item.id} text={['1st', '2nd', '3rd'][parseInt(item.slotId.split('-')[1])]} />
                            ))}
                        </div>
                    </div>
